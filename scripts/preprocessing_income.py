...
# Read in SA2 data
SA2 = pd.read_csv(f"{dir_name}{relative_dir}SA2_2016_AUST.csv")
...
sf = gpd.read_file(f"{dir_name}{relative_dir}SA2_shapefile/SA2_2016_AUST.shp")
...
# Read in income data
income = pd.read_excel(f"{dir_name}{relative_dir}income_distribution.xlsx", sheet_name="Table 2.4", header=[5,6])
...
history = pd.read_excel(f"{dir_name}{relative_dir}income_history.xlsx", sheet_name="Table 1.4", header=[5,6])
...
# Save history
vic_history.to_csv(f"{dir_name}{relative_dir}vic_income_history.csv")
...
for attribute in attributes:
    ...
    c.add_to(m)
    m.save(f"{dir_name}/plots/{attribute}_income.html")
    m

# Save data
vic_income.to_csv(f"{dir_name}{relative_dir}vic_income.csv")

print("Completed preprocessing income data")